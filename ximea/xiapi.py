...
class Image(XI_IMG):
    ...
    def get_image_data_numpy(self, invert_rgb_order=False):
        ...
        '''
        try:
            if self.frm == XI_IMG_FORMAT["XI_MONO8"].value or \
            self.frm == XI_IMG_FORMAT["XI_RAW8"].value :
                ...
                return numpy_data
            elif self.frm == XI_IMG_FORMAT["XI_MONO16"].value or \
            self.frm == XI_IMG_FORMAT["XI_RAW16"].value:
                ...
                return numpy_data    
            elif self.frm == XI_IMG_FORMAT["XI_RAW32"].value:
                c_array = c_ulong*self.width*self.height
                data = c_array()
                c_arr_ops.arr16bit(
                    c_int(self.height),
                    c_int(self.width),
                ...
                numpy_data = np.array(data, copy=False, dtype=np.uint16)
                return numpy_data    
            elif self.frm == XI_IMG_FORMAT["XI_RAW8X2"].value or \
            self.frm == XI_IMG_FORMAT["XI_RAW8X4"].value:
                c_array = c_ubyte*self.get_channel_count()*self.width*self.height
                data = c_array()
                c_arr_ops.arr8bit(
                    c_int(self.height),
                    c_int(self.get_channel_count()*self.width),
                    c_int(self.padding_x),
                    c_void_p(self.bp),
                    data
                    )
                numpy_data = np.array(data, copy=False, dtype=np.uint8)
                return numpy_data
            elif self.frm == XI_IMG_FORMAT["XI_RAW16X2"].value or \
            self.frm == XI_IMG_FORMAT["XI_RAW16X4"].value:
                c_array = c_ushort*self.get_channel_count()*self.width*self.height
                data = c_array()
                c_arr_ops.arr16bit(
                    c_int(self.height),
                    c_int(self.get_channel_count()*self.width),
                ...
                numpy_data = np.array(data, copy=False, dtype=np.uint16)
                return numpy_data    
            elif self.frm == XI_IMG_FORMAT["XI_RGB24"].value or \
            self.frm == XI_IMG_FORMAT["XI_RGB_PLANAR"].value:
    ...
    def get_channel_count(self):
        '''
        Return number (int) of channels per single pixel.
        '''
        if self.frm == XI_IMG_FORMAT["XI_MONO8"].value or \
        self.frm == XI_IMG_FORMAT["XI_RAW8"].value or \
        self.frm == XI_IMG_FORMAT["XI_MONO16"].value or \
        self.frm == XI_IMG_FORMAT["XI_RAW16"].value:
            return 1
        elif self.frm == XI_IMG_FORMAT["XI_RAW8X2"].value or \
        self.frm == XI_IMG_FORMAT["XI_RAW16X2"].value:
            return 2
        elif self.frm == XI_IMG_FORMAT["XI_RGB24"].value or \
		self.frm == XI_IMG_FORMAT["XI_RAW32"].value or \
        self.frm == XI_IMG_FORMAT["XI_RGB_PLANAR"].value:
            return 3
        elif self.frm == XI_IMG_FORMAT["XI_RAW8X4"].value or \
        self.frm == XI_IMG_FORMAT["XI_RAW16X4"].value or \
        self.frm == XI_IMG_FORMAT["XI_RGB32"].value: 
            return 4
        else:
            raise Xi_error(108)     #"Data format not supported"
...
#-------------------------------------------------------------------------------------------------------------------
    ...
    def set_gain_direct(self, gain):
        '''
        Gain in dBXI_PRM_GAIN
        '''
        self.set_param('gain:direct_update', gain)
    ...
    def get_wb_roi_offset_x(self):
        '''
        White balance offset XXI_PRM_WB_ROI_OFFSET_X
        '''
        return self.get_param('wb_roi_offset_x')

    def get_wb_roi_offset_x_maximum(self):
        '''
        White balance offset XXI_PRM_WB_ROI_OFFSET_X
        '''
        return self.get_param('wb_roi_offset_x:max')

    def get_wb_roi_offset_x_minimum(self):
        '''
        White balance offset XXI_PRM_WB_ROI_OFFSET_X
        '''
        return self.get_param('wb_roi_offset_x:min')

    def get_wb_roi_offset_x_increment(self):
        '''
        White balance offset XXI_PRM_WB_ROI_OFFSET_X
        '''
        return self.get_param('wb_roi_offset_x:inc')

    def set_wb_roi_offset_x(self, wb_roi_offset_x):
        '''
        White balance offset XXI_PRM_WB_ROI_OFFSET_X
        '''
        self.set_param('wb_roi_offset_x', wb_roi_offset_x)

    def get_wb_roi_offset_y(self):
        '''
        White balance offset YXI_PRM_WB_ROI_OFFSET_Y
        '''
        return self.get_param('wb_roi_offset_y')

    def get_wb_roi_offset_y_maximum(self):
        '''
        White balance offset YXI_PRM_WB_ROI_OFFSET_Y
        '''
        return self.get_param('wb_roi_offset_y:max')

    def get_wb_roi_offset_y_minimum(self):
        '''
        White balance offset YXI_PRM_WB_ROI_OFFSET_Y
        '''
        return self.get_param('wb_roi_offset_y:min')

    def get_wb_roi_offset_y_increment(self):
        '''
        White balance offset YXI_PRM_WB_ROI_OFFSET_Y
        '''
        return self.get_param('wb_roi_offset_y:inc')

    def set_wb_roi_offset_y(self, wb_roi_offset_y):
        '''
        White balance offset YXI_PRM_WB_ROI_OFFSET_Y
        '''
        self.set_param('wb_roi_offset_y', wb_roi_offset_y)

    def get_wb_roi_width(self):
        '''
        White balance widthXI_PRM_WB_ROI_WIDTH
        '''
        return self.get_param('wb_roi_width')

    def get_wb_roi_width_maximum(self):
        '''
        White balance widthXI_PRM_WB_ROI_WIDTH
        '''
        return self.get_param('wb_roi_width:max')

    def get_wb_roi_width_minimum(self):
        '''
        White balance widthXI_PRM_WB_ROI_WIDTH
        '''
        return self.get_param('wb_roi_width:min')

    def get_wb_roi_width_increment(self):
        '''
        White balance widthXI_PRM_WB_ROI_WIDTH
        '''
        return self.get_param('wb_roi_width:inc')

    def set_wb_roi_width(self, wb_roi_width):
        '''
        White balance widthXI_PRM_WB_ROI_WIDTH
        '''
        self.set_param('wb_roi_width', wb_roi_width)

    def get_wb_roi_height(self):
        '''
        White balance heightXI_PRM_WB_ROI_HEIGHT
        '''
        return self.get_param('wb_roi_height')

    def get_wb_roi_height_maximum(self):
        '''
        White balance heightXI_PRM_WB_ROI_HEIGHT
        '''
        return self.get_param('wb_roi_height:max')

    def get_wb_roi_height_minimum(self):
        '''
        White balance heightXI_PRM_WB_ROI_HEIGHT
        '''
        return self.get_param('wb_roi_height:min')

    def get_wb_roi_height_increment(self):
        '''
        White balance heightXI_PRM_WB_ROI_HEIGHT
        '''
        return self.get_param('wb_roi_height:inc')

    def set_wb_roi_height(self, wb_roi_height):
        '''
        White balance heightXI_PRM_WB_ROI_HEIGHT
        '''
        self.set_param('wb_roi_height', wb_roi_height)
...
#-------------------------------------------------------------------------------------------------------------------
    ...
    def get_timestamp(self):
        '''
        Current value of the device timestamp counterXI_PRM_TIMESTAMP
        '''
        return self.get_param('timestamp')

    def get_timestamp_maximum(self):
        '''
        Current value of the device timestamp counterXI_PRM_TIMESTAMP
        '''
        return self.get_param('timestamp:max')

    def get_timestamp_minimum(self):
        '''
        Current value of the device timestamp counterXI_PRM_TIMESTAMP
        '''
        return self.get_param('timestamp:min')

    def get_timestamp_increment(self):
        '''
        Current value of the device timestamp counterXI_PRM_TIMESTAMP
        '''
        return self.get_param('timestamp:inc')
...
#-------------------------------------------------------------------------------------------------------------------
    ...
    def get_lens_aperture_index(self):
        '''
        Current aperture index as reported by lens.XI_PRM_LENS_APERTURE_INDEX
        '''
        return self.get_param('lens_aperture_index')

    def get_lens_aperture_index_maximum(self):
        '''
        Current aperture index as reported by lens.XI_PRM_LENS_APERTURE_INDEX
        '''
        return self.get_param('lens_aperture_index:max')

    def get_lens_aperture_index_minimum(self):
        '''
        Current aperture index as reported by lens.XI_PRM_LENS_APERTURE_INDEX
        '''
        return self.get_param('lens_aperture_index:min')

    def get_lens_aperture_index_increment(self):
        '''
        Current aperture index as reported by lens.XI_PRM_LENS_APERTURE_INDEX
        '''
        return self.get_param('lens_aperture_index:inc')

    def set_lens_aperture_index(self, lens_aperture_index):
        '''
        Current aperture index as reported by lens.XI_PRM_LENS_APERTURE_INDEX
        '''
        self.set_param('lens_aperture_index', lens_aperture_index)
...
#-------------------------------------------------------------------------------------------------------------------
    ...
    def get_concat_img_transport_img_offset(self):
        '''
        Offset between images data in transport buffer when feature Concatenated Images in One Buffer is enabledXI_PRM_CONCAT_IMG_TRANSPORT_IMG_OFFSET
        '''
...
#-------------------------------------------------------------------------------------------------------------------
    ...
    def get_proc_num_threads(self):
        '''
        Number of threads per image processorXI_PRM_PROC_NUM_THREADS
        '''
        return self.get_param('proc_num_threads')

    def get_proc_num_threads_maximum(self):
        '''
        Number of threads per image processorXI_PRM_PROC_NUM_THREADS
        '''
        return self.get_param('proc_num_threads:max')

    def get_proc_num_threads_minimum(self):
        '''
        Number of threads per image processorXI_PRM_PROC_NUM_THREADS
        '''
        return self.get_param('proc_num_threads:min')

    def get_proc_num_threads_increment(self):
        '''
        Number of threads per image processorXI_PRM_PROC_NUM_THREADS
        '''
        return self.get_param('proc_num_threads:inc')

    def set_proc_num_threads(self, proc_num_threads):
        '''
        Number of threads per image processorXI_PRM_PROC_NUM_THREADS
        '''
        self.set_param('proc_num_threads', proc_num_threads)

    def get_proc_engine(self):
        '''
        Set processing engineXI_PRM_PROC_ENGINE
        '''
        return self.get_param('proc_engine')

    def get_proc_engine_maximum(self):
        '''
        Set processing engineXI_PRM_PROC_ENGINE
        '''
        return self.get_param('proc_engine:max')

    def get_proc_engine_minimum(self):
        '''
        Set processing engineXI_PRM_PROC_ENGINE
        '''
        return self.get_param('proc_engine:min')

    def get_proc_engine_increment(self):
        '''
        Set processing engineXI_PRM_PROC_ENGINE
        '''
        return self.get_param('proc_engine:inc')

    def set_proc_engine(self, proc_engine):
        '''
        Set processing engineXI_PRM_PROC_ENGINE
        '''
        self.set_param('proc_engine', proc_engine)
...
#-------------------------------------------------------------------------------------------------------------------
    ...
    def get_dp_unit_selector(self):
        '''
        Data Pipe Unit Selector.XI_PRM_DP_UNIT_SELECTOR
        '''
        return self.get_param('dp_unit_selector')

    def get_dp_unit_selector_maximum(self):
        '''
        Data Pipe Unit Selector.XI_PRM_DP_UNIT_SELECTOR
        '''
        return self.get_param('dp_unit_selector:max')

    def get_dp_unit_selector_minimum(self):
        '''
        Data Pipe Unit Selector.XI_PRM_DP_UNIT_SELECTOR
        '''
        return self.get_param('dp_unit_selector:min')

    def get_dp_unit_selector_increment(self):
        '''
        Data Pipe Unit Selector.XI_PRM_DP_UNIT_SELECTOR
        '''
        return self.get_param('dp_unit_selector:inc')

    def set_dp_unit_selector(self, dp_unit_selector):
        '''
        Data Pipe Unit Selector.XI_PRM_DP_UNIT_SELECTOR
        '''
        self.set_param('dp_unit_selector', dp_unit_selector)

    def get_dp_proc_selector(self):
        '''
        Data Pipe Processor Selector.XI_PRM_DP_PROC_SELECTOR
        '''
        return self.get_param('dp_proc_selector')

    def get_dp_proc_selector_maximum(self):
        '''
        Data Pipe Processor Selector.XI_PRM_DP_PROC_SELECTOR
        '''
        return self.get_param('dp_proc_selector:max')

    def get_dp_proc_selector_minimum(self):
        '''
        Data Pipe Processor Selector.XI_PRM_DP_PROC_SELECTOR
        '''
        return self.get_param('dp_proc_selector:min')

    def get_dp_proc_selector_increment(self):
        '''
        Data Pipe Processor Selector.XI_PRM_DP_PROC_SELECTOR
        '''
        return self.get_param('dp_proc_selector:inc')

    def set_dp_proc_selector(self, dp_proc_selector):
        '''
        Data Pipe Processor Selector.XI_PRM_DP_PROC_SELECTOR
        '''
        self.set_param('dp_proc_selector', dp_proc_selector)

    def get_dp_param_selector(self):
        '''
        Data Pipe Processor parameter Selector.XI_PRM_DP_PARAM_SELECTOR
        '''
        return self.get_param('dp_param_selector')

    def get_dp_param_selector_maximum(self):
        '''
        Data Pipe Processor parameter Selector.XI_PRM_DP_PARAM_SELECTOR
        '''
        return self.get_param('dp_param_selector:max')

    def get_dp_param_selector_minimum(self):
        '''
        Data Pipe Processor parameter Selector.XI_PRM_DP_PARAM_SELECTOR
        '''
        return self.get_param('dp_param_selector:min')

    def get_dp_param_selector_increment(self):
        '''
        Data Pipe Processor parameter Selector.XI_PRM_DP_PARAM_SELECTOR
        '''
        return self.get_param('dp_param_selector:inc')

    def set_dp_param_selector(self, dp_param_selector):
        '''
        Data Pipe Processor parameter Selector.XI_PRM_DP_PARAM_SELECTOR
        '''
        self.set_param('dp_param_selector', dp_param_selector)

    def get_dp_param_value(self):
        '''
        Data Pipe processor parameter valueXI_PRM_DP_PARAM_VALUE
        '''
        return self.get_param('dp_param_value')

    def get_dp_param_value_maximum(self):
        '''
        Data Pipe processor parameter valueXI_PRM_DP_PARAM_VALUE
        '''
        return self.get_param('dp_param_value:max')

    def get_dp_param_value_minimum(self):
        '''
        Data Pipe processor parameter valueXI_PRM_DP_PARAM_VALUE
        '''
        return self.get_param('dp_param_value:min')

    def get_dp_param_value_increment(self):
        '''
        Data Pipe processor parameter valueXI_PRM_DP_PARAM_VALUE
        '''
        return self.get_param('dp_param_value:inc')

    def set_dp_param_value(self, dp_param_value):
        '''
        Data Pipe processor parameter valueXI_PRM_DP_PARAM_VALUE
        '''
